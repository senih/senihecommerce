...
NLSMENU.setItemText = function (key, tx) {
  ...
  var mnItem = this.items[intKey];
  mnItem.capt=tx;
  var oD=NlsGetElementById(intKey+"x2");
...
NLSMENU.renderMenu = function() {
  ...
  for (var i=0; i<this.srItems.length; i++) {
    it=this.srItems[i];
    if (!nlsMenu[it.subMenuId])it.subMenuId="";
  ...
  if (!this.isMenubar &&  nls_isIE && !nls_isIE5 && nlsMenuMgr[this.mgrId].flowOverFormElement) { 
    smenu += "<iframe id='"+this.mId+"ifrm' scrolling='no' frameborder=0 width='1' height='1' style='position:absolute;top:0px;left:0px;z-index:-1;filter:alpha(opacity=0)' src='"+this.imgPath+"img/blank.gif"+"'></iframe>"; 
  }
  smenu+="</div>";
