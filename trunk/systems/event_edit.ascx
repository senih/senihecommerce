...
<script runat=server>
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsNothing(GetUser) Then
            ...
            nEventID = CInt(Request.QueryString("id"))

            sqlCmd = New SqlCommand("SELECT * from events where event_id = @event_id", oConn)
            sqlCmd.Parameters.Add("@event_id", SqlDbType.Int).Value = nEventID
            oConn.Open()
            oDataReader = sqlCmd.ExecuteReader()
            If oDataReader.Read() Then

                If Not GetUser().ToString = oDataReader("author").ToString Then 'security
                    Response.Redirect(Me.LinkWorkspaceEvents)
                End If

                txtTitle.Text = oDataReader("title")
                txtDescription.Text = oDataReader("description")
                ckbAlldDay.Checked = oDataReader("is_allday")
                If ckbAlldDay.Checked Then
                    panelTime.Style.Add("display", "none")
                    panelTime2.Style.Add("display", "none")
                End If
                'txtReminder.Text = oDataReader("reminder")
                'ddlReminder.SelectedValue = oDataReader("reminder_id")
                txtPage.Text = oDataReader("page")
                nRepeat = oDataReader("repeat")
                If nRepeat = 1 Then
                    panelUntil.Style.Add("display", "none")
                Else
                    panelUntil.Style.Add("display", "")
                End If
                ddlType.SelectedValue = oDataReader("type")

                dtFromDate = CType(oDataReader("from_date"), DateTime)
                dtToDate = CType(oDataReader("to_date"), DateTime)
                dtUntil = CType(oDataReader("until"), DateTime)

                ddlMonth.SelectedValue = dtFromDate.Month
                ddlDay.SelectedValue = dtFromDate.Day
                ddlYear.SelectedValue = dtFromDate.Year
                ddlMonth2.SelectedValue = dtToDate.Month
                ddlDay2.SelectedValue = dtToDate.Day
                ddlYear2.SelectedValue = dtToDate.Year
                ddlMonth3.SelectedValue = dtUntil.Month
                ddlDay3.SelectedValue = dtUntil.Day
                ddlYear3.SelectedValue = dtUntil.Year
                ddlHour.SelectedValue = dtFromDate.Hour
                ddlMinute.SelectedValue = dtFromDate.Minute
                ddlHour2.SelectedValue = dtToDate.Hour
                ddlMinute2.SelectedValue = dtToDate.Minute
            End If
            oDataReader.Close()
            oConn.Close()

            If nRepeat = 0 Then
                ddlRepeat.Enabled = False
                panelUntil.Style.Add("display", "none")
            Else
                ddlRepeat.SelectedValue = nRepeat
            End If
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If Not (IsNothing(GetUser())) Then
            ...
            If Not (nRepeat = 0) Then ' edit master event, delete all event first, and create again
                oConn.Open()

                'Delete
                strSQL = "DELETE FROM events WHERE event_id = @parent_event_id OR parent_id = @parent_event_id"
                sqlCmd = New SqlCommand(strSQL, oConn)
                sqlCmd.CommandType = CommandType.Text
                sqlCmd.Parameters.Add("@parent_event_id", SqlDbType.Int).Value = nEventID
                ...
                sqlCmd.Parameters.Add("@from_date", SqlDbType.DateTime).Value = dtFromDate ' Sebelumnya SqlDbType.NVarChar

                sqlCmd.Parameters.Add("@to_date", SqlDbType.DateTime).Value = dtToDate ' Sebelumnya SqlDbType.NVarChar
                sqlCmd.Parameters.Add("@repeat", SqlDbType.Int).Value = ddlRepeat.SelectedValue
                sqlCmd.Parameters.Add("@until", SqlDbType.DateTime).Value = dtUntilDate ' Sebelumnya SqlDbType.NVarChar
                ...
                If Not (ddlRepeat.SelectedValue = 1) Then 'Selain repeat=none
                    ...
                    Do While (dtFromDate2 <= dtUntilDate)
                        ...
                        End If
                        'sqlCmd.Parameters.Item("@root_id").Value = Me.RootID 'value sama dengan yang diatas
            ...
            Else 'update recursive
                strSQL = "UPDATE events SET  title = @title , description = @description, is_allday = @is_allday, from_date = @from_date ," & _
                                     " to_date = @to_date ,reminder = @reminder , reminder_id = @reminder_id , page = @page ,author = @author ," & _
                                     " type = @type" & _
...
<asp:Panel ID="panelEditEvent" runat="server" Visible="False">
<table>
<tr>
    ...
    <td> 
        ...
        <td stle="height:30px">
            <asp:Panel ID="panelTime" runat="server">
                <table cellpadding=0 cellspacing=0>
                <tr>
                <td>&nbsp;<asp:Label ID="lblAt" meta:resourcekey="lblAt" runat="server" Text="at"></asp:Label> &nbsp;</td>
                <td>
...
<tr>
    ...
    <td>
        ...
        <td>
            ...
            <tr>
            <td>
                <asp:Label ID="lblUntil" meta:resourcekey="lblUntil" runat="server" Text="Until"></asp:Label> :&nbsp;</td>
            <td>
                <table cellpadding=0 cellspacing=0>
                <tr>
                <td></td>
        ...
        function repeat(idDay1,idMonth1,idYear1,idDay2,idMonth2,idYear2,idDay3,idMonth3,idYear3)
            ...
            validateDate(idDay3,idMonth3,idYear2);//yus
            }
        </script>
        <script>
...
<%--<tr>
    ...
    <td>:</td>
    <td>
        <asp:TextBox ID="txtReminder" runat="server" Width="30px"></asp:TextBox>&nbsp;
...
<tr>
    <td>
        <asp:Label ID="lblLinkToPage" meta:resourcekey="lblLinkToPage" runat="server" Text="Link to page"></asp:Label></td>
    <td>:</td>
    <td>
        <asp:TextBox ID="txtPage" runat="server"></asp:TextBox> 
        <asp:Label ID="lblEg" meta:resourcekey="lblEg" runat="server" Text="(eg. event.aspx)"></asp:Label>
    </td>
</tr>
<tr>
