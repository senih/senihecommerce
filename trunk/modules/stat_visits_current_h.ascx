...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        oCommand = New SqlCommand("SET LANGUAGE us_english SELECT COUNT(Expr1) AS tot, 'this' as mo FROM (SELECT ip AS Expr1 FROM page_views WHERE (MONTH(date_stamp) = @thismonth) AND (YEAR(date_stamp) = @year1) AND (page_id = @page_id) GROUP BY ip) AS derivedtbl_1 " & _
                  "union " & _
                  "SELECT COUNT(Expr1) AS tot, 'last' as mo FROM (SELECT ip AS Expr1 FROM page_views WHERE (MONTH(date_stamp) = @lastmonth) AND (YEAR(date_stamp) = @year2) AND (page_id = @page_id) GROUP BY ip) AS derivedtbl_1 " & _
                  "union " & _
                  "SELECT  COUNT(ip) AS tot, 'overall' as mo FROM (SELECT page_id, ip FROM page_views GROUP BY page_id, ip) AS derivedtbl_1 WHERE page_id=@page_id GROUP BY page_id", oConn)
        ...
        Dim nC(7) As Integer
        Dim dD(7) As Date
        oCommand = New SqlCommand("SET LANGUAGE us_english SELECT date_stamp, COUNT(ip) AS count FROM (SELECT date_stamp, ip FROM page_views WHERE (page_id = 1 and date_stamp>=@start) GROUP BY date_stamp, ip) AS derivedtbl_1 GROUP BY date_stamp", oConn)
        oCommand.CommandType = CommandType.Text
        oCommand.Parameters.Add("@page_id", SqlDbType.Int).Value = Me.PageID
        ...
        Dim bStatHorizontal As Boolean = True
...
<asp:Panel ID="panelStat" CssClass="boxStatHorizontal" runat="server">
</asp:Panel>
