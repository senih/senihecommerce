...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        oCommand = New SqlCommand("SET LANGUAGE us_english SELECT COUNT(Expr1) AS tot, 'this' as mo FROM (SELECT ip AS Expr1 FROM page_views WHERE (MONTH(date_stamp) = @thismonth) AND (YEAR(date_stamp) = @year1) GROUP BY ip) AS derivedtbl_1 " & _
            "union " & _
            "SELECT COUNT(Expr1) AS tot, 'last' as mo FROM (SELECT ip AS Expr1 FROM page_views WHERE (MONTH(date_stamp) = @lastmonth) AND (YEAR(date_stamp) = @year2) GROUP BY ip) AS derivedtbl_1 " & _
            "union " & _
            "SELECT COUNT(Expr1) AS tot, 'overall' as mo FROM (SELECT ip AS Expr1 FROM page_views GROUP BY ip) AS derivedtbl_1", oConn)
        ...
        Dim nC(7) As Integer
        Dim dD(7) As Date
        oCommand = New SqlCommand("SET LANGUAGE us_english SELECT date_stamp, COUNT(ip) AS tot FROM (SELECT date_stamp, ip FROM page_views WHERE (date_stamp >= @start) GROUP BY date_stamp, ip) AS derivedtbl_1 GROUP BY date_stamp ORDER BY date_stamp", oConn)
        oCommand.CommandType = CommandType.Text
        ...
        While oDataReader.Read
            ...
            nStatCount = CInt(oDataReader("tot"))
