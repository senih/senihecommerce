...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        oCommand = New SqlCommand("SET LANGUAGE us_english select sum(count) as tot, 'this' as mo from pages_views_count_daily where month(date_stamp) = @thismonth and year(date_stamp)=@year1 " & _
            "union " & _
            "select sum(count) as tot, 'last' as mo from pages_views_count_daily where month(date_stamp) = @lastmonth and year(date_stamp)=@year2 " & _
            "union " & _
            "select sum(total) as tot, 'overall' as mo from pages_views_count", oConn)
        ...
        Dim nC(7) As Integer
        Dim dD(7) As Date
        oCommand = New SqlCommand("SET LANGUAGE us_english SELECT date_stamp, SUM(count) AS tot FROM pages_views_count_daily WHERE date_stamp >=@start GROUP BY date_stamp ORDER BY date_stamp", oConn)
        oCommand.CommandType = CommandType.Text
