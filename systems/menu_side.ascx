...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        While oDataReader.Read()
            ...
            If bShowMenu Then
                If Not bIsSys And Not nLevel = 0 Then 'nLevel=0 adalah level dari selected TAB

                    If nCurrLevel = 0 Then
                        sMenu += "<ul class=""mnuLevel" & nLevel & """><li>"
                    ElseIf nCurrLevel < nLevel Then
                        sMenu += "<ul class=""mnuLevel" & nLevel & """><li>"
                    ElseIf nCurrLevel > nLevel Then
                        Dim i As Integer
                        For i = 1 To nCurrLevel - nLevel
                            sMenu += "</ul>"
                        Next
                        sMenu += "<li>"
                    Else
                    ...
                    If oColPath.Contains(nPgId.ToString) Then
                        If nPgId = Me.PageID Then
                            'selected
                            sMenu += "<span class=""itemSelected"">" & sTtl & "</span>"
                        Else
                            'path
                            sMenu += "<a href=""" & sFileUrl & """ " & sTarget & ">" & sTtl & "</a>"
                        End If
                    Else
                        'normal
                        sMenu += "<a href=""" & sFileUrl & """ " & sTarget & ">" & sTtl & "</a>"
        ...
        If sMenu <> "" Then
            Dim i As Integer
            For i = 1 To nCurrLevel
                sMenu += "</ul>"
            Next
            panelMenu.Controls.Add(New LiteralControl("<div id=""divSide"">" & sMenu & "</div>"))
        End If
